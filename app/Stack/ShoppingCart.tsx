...
interface ShoppingCartProps {
  ...
  onRemoveItem: (id: string) => void; // New prop for removing item
}
  ...
  return (
    <>
      <div className={`fixed top-0 right-0 h-full w-full md:w-2/5 bg-customLightGold shadow-lg transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 z-50`}>
        ...
        <div className="p-4 bg-customLightBlue h-[calc(90%-128px)] overflow-y-auto"> {/* Adjusted height */}
          <div className="p-4 grid gap-4"> {/* Adjusted grid and gap */}
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between bg-customLightGold p-4 shadow rounded-lg">
                <div className="relative w-16 h-16"> {/* Wrapper div with fixed size */}
                  <Image src={item.shoppingCartImage} alt={item.name} layout="fill" objectFit="cover" className="" /> {/* Square image with cover */}
                </div>
